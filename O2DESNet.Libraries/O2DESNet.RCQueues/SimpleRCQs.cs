...
namespace O2DESNet.RCQueues
{
    public class SimpleRCQs : Sandbox<SimpleRCQs.Statics>
    {
        public class Statics : IAssets
        {
                ...
                foreach (var r in File.ReadAllLines(string.Format("{0}\\{1}.csv", dir, Key.Arrivals)))
                {
                    ...
                    switch (line[0])
                    {
                        ...
                        case Key.SeasonalFactors_HoursOfDay:
                            simpleRCQs.Generator.SeasonalFactorsHoursOfDay = line.GetRange(1, line.Count - 1)
                                .Select(str => double.Parse(str)).ToList();
                            break;
                        case Key.SeasonalFactors_DaysOfWeek:
                            simpleRCQs.Generator.SeasonalFactorsDaysOfWeek = line.GetRange(1, line.Count - 1)
                                .Select(str => double.Parse(str)).ToList();
                            break;
                        case Key.SeasonalFactors_DaysOfMonth:
                            simpleRCQs.Generator.SeasonalFactorsDaysOfMonth = line.GetRange(1, line.Count - 1)
                                .Select(str => double.Parse(str)).ToList();
                            break;
                        case Key.SeasonalFactors_MonthsOfYear:
                            simpleRCQs.Generator.SeasonalFactorsMonthsOfYear = line.GetRange(1, line.Count - 1)
                                .Select(str => double.Parse(str)).ToList();
                            break;
                        case Key.SeasonalFactors_Years:
                            simpleRCQs.Generator.SeasonalFactorsYears = line.GetRange(1, line.Count - 1)
                                .Select(str => double.Parse(str)).ToList();
                            break;
                    }
                }
            ...
            {
                ResourceDict.Add(id, new Resource(id, id, capacity) { Description = description });
