...
namespace O2DESNet.RCQueues
{
    public class RCQsModel : Sandbox<IRCQsModelStatics>, IRCQsModel
    {
        ...
        public IReadOnlyDictionary<IResource, double> ResourceQuantity_Locked
        {
            get { return _resourceQuantity_Locked.AsReadOnly(); }
        }
        private readonly Dictionary<IResource, double> _resourceQuantity_Locked;
        #endregion
        ...
        private double RemainingCapcity(IResource res, Batch toMove)
        {
            ...
            // fix: returns true if the next activity shares the same resource
            bool nextActWithSameResource = false;
            foreach (var curr in toMove.Select(load => _loadToBatch_Current[load]).Distinct())
            {
                ...
                nextActWithSameResource = true;
            }

            // fix: if next activity shares the same resource,
            // the resource will be given priority to the pending activity
            // and not the resource pending to be locked.
            if (_resourceQuantity_PendingLock.ContainsKey(res) && nextActWithSameResource)
            {
                return remaining;
            }
            else
        ...
        private Dictionary<IResource, double> ReleaseResources(IEnumerable<Batch> currents)
        {
            ...
            // fix: Deleted these codes as for SATS model, all successive activities that share the same resources will need to be completed first before it is released and locked.
        ...
        protected void RecallForPending(IEnumerable<IResource> released)
        {
            ...
            while (activities_PendingToReview.Count > 0)
            {
                ...
                {
                    // //Console.WriteLine("============================================================pending activity removed as batch is not pending. Activity: {0}================================================================", moveTo.Activity);
                    ...
                    // fix: this should throw an exception instead of only deleting the activity
        ...
        private void AttemptToStart(Batch moveTo)
        {
            ...
            if (request != null)
            {
                ...
                // used in fix (see line 652): This allows the resources that are shared by successive activities to flow through 
                // the next activity without it being released by the activity itself.                  
                ...
                foreach (var i in _batchToAllocation[moveTo].ResourceQuantity_Aggregated)
                {
                    ...
                    //act.Name == "Uld_BUPI_OAL_Park at Ramp" 
                    // || act.Name == "Uld_BUPI_SQ_Park at Ramp" 
                    // || act.Name == "Uld_OALU_Check Cargo" 
                    // || act.Name == "Uld_BUPI_Check Cargo" 
                    // if (act.Name == "Uld_OALU_Introduce to System" 
                    // // || act.Name == "Uld_SQB_Check Cargo" 
                    // || act.Name == "Uld_SQB_Introduce to System" 
                    // act.Name == "Uld_SQU_Check Cargo" 
                    // || act.Name == "Uld_SQU_Introduce to System" 
                    // // || act.Name == "Uld_SQL_Check Cargo"
                    // ) {
                    //     //Console.WriteLine("Sufficient resource - Activity: {0}, Resource used: {1}, HC occupied: {2}, HC available: {3}", act.Name, res.Name, _resourceHC_Occupied[res].LastCount, _resourceHC_Available[res].LastCount);
                    // }
                }
                ...
                {
                    var batch = _activityToBatchTimes_Pending[moveTo.Activity].First().Batch;
                    Schedule(() => AttemptToStart(batch));
                }


                // fix: added this foreach loop that is originally in ReleaseResources.
                // This will prevent the resource to be locked before resource occupation (line 546)
                // of successive activities sharing the same resource
                foreach (var res in released.Keys.ToList())
                {
                    if (_resourceQuantity_PendingLock[res] > 0)
                    {
            ...
            else
            {
                if (moveTo.Phase != BatchPhase.Pending)
                {
                    ...
                    foreach (var res in _activityToResources[moveTo.Activity])
                    {
                        //(moveTo.Activity.Name == "Uld_BUPI_OAL_Park at Ramp" 
                        // || moveTo.Activity.Name == "Uld_BUPI_SQ_Park at Ramp" 
                        // || moveTo.Activity.Name == "Uld_OALU_Check Cargo" 
                        // || moveTo.Activity.Name == "Uld_BUPI_Check Cargo" 
                        if (moveTo.Activity.Name == "Uld_OALU_Introduce to System"
                        // // || moveTo.Activity.Name == "Uld_SQB_Check Cargo" 
                        || moveTo.Activity.Name == "Uld_SQB_Introduce to System"
                        // moveTo.Activity.Name == "Uld_SQU_Check Cargo" 
                        || moveTo.Activity.Name == "Uld_SQU_Introduce to System"
                        // // || moveTo.Activity.Name == "Uld_SQL_Check Cargo"
                        )
                        {
                            //Console.WriteLine("Insufficient Resource - Activity: {0}, Resource used: {1}, HC occupied: {2}, HC available: {3}", moveTo.Activity.Name, res.Name, _resourceHC_Occupied[res].LastCount, _resourceHC_Available[res].LastCount);
                        }
                        UpdHourCounter_Resource_Pending(res);
        ...
        public void RequestToLock(IResource resource, double quantity)
        {
            ...
            string resource_request_to_lock = String.Format("Resource request to lock. Resource Name: {0}, Quantity to lock: {1}, Pending resource lock before: {2}, Actual quantity locked before: {3}, Dynamic capacity before: {4}, Occupied capaciy before: {5}.", resource.Name, quantity, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_request_to_lock);
            ...
            string resource_request_to_lock_after = String.Format("Resource Name: {0}. Pending resource lock after: {1}, Actual quantity locked after: {2}, Dynamic capacity after: {3}, Occupied capacity after: {4}.", resource.Name, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_request_to_lock_after);
        ...
        public void RequestToUnlock(IResource resource, double quantity)
        {
            ...
            string resource_request_to_unlock = String.Format("Resource request to unlock. Resource Name: {0}, Quantity unlock: {1}, Pending resource lock before: {2}, Actual quantity locked before: {3}, Dynamic Capacity before: {4}, Occupied Capacity before: {5}.", resource.Name, quantity, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_request_to_unlock);
            var qtt = quantity;
            ...
            _resourceQuantity_Locked[resource] -= qtt;
            string resource_request_to_unlock_after = String.Format("Resource Name: {0}. Pending resource lock after: {1}, Actual quantity locked after: {2}, Dynamic Capacity after: {3}, Occupied Capacity after: {4}.", resource.Name, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_request_to_unlock_after);
        ...
        /// <summary>
        /// Attempt to lock resource for the pending quantity
        /// </summary>
        private double AttemptToLock(IResource resource)
        {
            ...
            string resource_attempt_to_lock = String.Format("Resource attempt to lock. Resource Name: {0}, Quantity lock: {1}, Pending resource lock before request: {2}, Actual quantity locked before: {3}, Dynamic Capacity before: {4}, Occupied Capacity before: {5}.", resource.Name, quantity, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_attempt_to_lock);
            _resourceHC_Available[resource].ObserveChange(-quantity);
            _resourceQuantity_PendingLock[resource] -= quantity;
            _resourceQuantity_Locked[resource] += quantity;
            UpdateHourCounter_ResourcePendingLock(resource);
            string resource_attempt_to_lock_after = String.Format("Resource attempt to lock. Resource Name: {0}, Pending resource lock after request: {1}, Actual quantity locked after: {2}, Dynamic Capacity after: {3}, Occupied Capacity after: {4}.", resource.Name, _resourceQuantity_PendingLock[resource], _resourceQuantity_Locked[resource], _resourceHC_DynamicCapacity[resource].LastCount, _resourceHC_Occupied[resource].LastCount);
            //Console.WriteLine(resource_attempt_to_lock_after);
            if (quantity > 0)
            {
        ...
        public void UpdateHCPausing(IResource resource, int change)
        {
            //Debug.WriteLine("RCQsModel UpdateHCPausing: Resource: " + resource.Id + " Change: " + change.ToString());

            var dayOfWeek = ClockTime.DayOfWeek;

            if (change > 0)
            {
                //var utilRate = (_resourceHC_Active[resource].LastCount + change) / _resourceHC_DynamicCapacity[resource].LastCount;
                //if (utilRate >= 1) _resourceHC_Active[resource].ObserveCount(_resourceHC_DynamicCapacity[resource].LastCount);
                //else _resourceHC_Active[resource].ObserveChange(change);
                _resourceHC_Active[resource].ObserveChange(change);
            }
            if (change < 0)
            {
                _resourceHC_Active[resource].ObserveCount(0);
            }
            //Debug.WriteLine("RCQsModel UpdateHCPausing Resource: " + resource.Id + "Util Rate: " + _resourceHC_Active[resource].LastCount / _resourceHC_DynamicCapacity[resource].LastCount + " " + ClockTime);
            //Debug.WriteLine("RCQsModel UpdateHCPausing Resource HC Active for Resource: " + resource.Id + " Quantity: " + ResourceHC_Active[resource].LastCount);
            //Debug.WriteLine("RCQsModel UpdateHCPausing Resource HC Dynamic Capacity for Resource: " + resource.Id + " Quantity: " + ResourceHC_DynamicCapacity[resource].LastCount);
            //Debug.WriteLine("RCQsModel UpdateHCPausing Resource Utilisation Rate for Resource: " + resource.Id + " Rate: " + ResourceHC_Active[resource].LastCount / ResourceHC_DynamicCapacity[resource].LastCount);
        }
        ...
        public RCQsModel(IRCQsModelStatics assets, int seed, string id = null) : base(assets, seed, id)
        {
            ...
            _resourceQuantity_Locked = Assets.Resources.ToDictionary(res => res, res => 0d);
