    ...
    public sealed class ExcelPicture : ExcelDrawing
    {
            ...
            base(drawings, node, "xdr:pic/xdr:nvPicPr/xdr:cNvPr/@name")
        {
            ...
            if (picNode != null)
            {
                ...
                _image = Image.Load(Part.GetStream(), out IImageFormat format);

                byte[] iby = ImageCompat.GetImageAsByteArray(_image, format);
            ...
            base(drawings, node, "xdr:pic/xdr:nvPicPr/xdr:cNvPr/@name")
        {
            ...
            _image = Image.Load(imagestream, out IImageFormat format);

            var img = ImageCompat.GetImageAsByteArray(_image, format);
        ...
        internal static IImageEncoder GetImageFormat(string contentType)
        {
            switch (contentType.ToLower(CultureInfo.InvariantCulture))
            {
                case "image/bmp":
        ...
        private string SavePicture(Image image)
        {
            ...
            var ii = _drawings._package.AddImage(bytes);
